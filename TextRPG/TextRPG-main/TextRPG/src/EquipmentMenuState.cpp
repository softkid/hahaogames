...
constexpr int EQUIP_SIZE = 52;
constexpr int STAT_PREDICT_X_POS = 75;
constexpr int STAT_VAL_X_POS = 50;
constexpr int STAT_LABEL_X_POS = 30;
constexpr int STAT_LABEL_START_Y_POS = 23;

void EquipmentMenuState::DrawEquipment()
{
	// Draw Opening Bar
	m_Context.console.DrawPanelHorz(m_PanelBarX - 1, 1, PANEL_BARS + 2, YELLOW);
	int menu_x_pos = m_CenterScreenW - (EQUIP_SIZE / 2);
	m_Context.console.Write(menu_x_pos, 2, L" _____            _                            _  ", GREEN);
	m_Context.console.Write(menu_x_pos, 3, L"| ____|__ _ _   _(_)_ __  _ __ ___   ___ _ __ | |_ ", GREEN);
	m_Context.console.Write(menu_x_pos, 4, L"|  _| / _` | | | | | '_ \\| '_ ` _ \\ / _ \\ '_ \\| __|", GREEN);
	m_Context.console.Write(menu_x_pos, 5, L"| |__| (_| | |_| | | |_) | | | | | |  __/ | | | |_", GREEN);
	m_Context.console.Write(menu_x_pos, 6, L"|_____\\__, |\\__,_|_| .__/|_| |_| |_|\\___|_| |_|\\__|", GREEN);
	m_Context.console.Write(menu_x_pos, 7, L"         |_|       |_|                             ", GREEN);

	m_Context.console.DrawPanelHorz(m_PanelBarX - 1, 9, PANEL_BARS + 2, YELLOW);
	m_Context.console.DrawPanelHorz(m_PanelBarX - 1, 11, PANEL_BARS + 2, YELLOW);
	m_Context.console.DrawPanelHorz(m_PanelBarX - 1, 13, PANEL_BARS + 2, YELLOW);

	// Player holder
	m_Context.console.DrawPanelHorz(m_PanelBarX - 1, (m_ScreenHeight - 15), PANEL_BARS + 2, YELLOW);
	m_Context.console.DrawPanelHorz(m_PanelBarX - 1, (m_ScreenHeight - 17), PANEL_BARS + 2, YELLOW);
	// Draw Closing Bar
	m_Context.console.DrawPanelHorz(m_PanelBarX - 1, (m_ScreenHeight - 2), PANEL_BARS + 2, YELLOW);

	// Draw Right bar
	m_Context.console.DrawPanelVert(m_PanelBarX - 1, 2, 44, YELLOW);
	m_Context.console.DrawPanelVert(m_PanelBarX + PANEL_BARS, 2, 44, YELLOW);

	m_Context.console.Write(30, 21, L"ATTRIBUTES", YELLOW);
	m_Context.console.Write(30, 22, L"==========", YELLOW);
}

void EquipmentMenuState::DrawPlayerInfo()
{
	const auto& name = m_Player.GetName();
	m_Context.console.Write(m_CenterScreenW - static_cast<int>(name.size() / 2), 12, name);
	...
	for (const auto& [ stat, value ] : stats_list)
	{
		...
		m_Context.console.Write(STAT_LABEL_X_POS, STAT_LABEL_START_Y_POS + i, stat);
		m_Context.console.Write(STAT_VAL_X_POS, STAT_LABEL_START_Y_POS + i, std::to_wstring(value + mod_value));
...
void EquipmentMenuState::DrawStatPrediction()
{
	...
	{
		m_Context.console.Write(75, m_DiffPosY, L"    ");
		return;
	}
	...
	m_Context.console.Write(STAT_PREDICT_X_POS, m_DiffPosY, diff_dir + L" " + std::to_wstring(abs_diff_val), diff_color);
}
...
void EquipmentMenuState::DrawStatModifier(int x, int y, const std::wstring& stat, int value)
{
	...
	if (index > data.size() - 1)
	{
		m_Context.console.Write(x, m_PrevStatModPos, L"    ");
		return;
	}
	...
	{
		m_Context.console.Write(x, y, L"    ");
		return;
	}

	const std::wstring stat_val = std::to_wstring(stat_modifier.statModifierVal);
	m_Context.console.Write(x, y, L" + " + stat_val, GREEN);
	m_PrevStatModPos = y;
}
...
void EquipmentMenuState::RenderEquip(int x, int y, std::shared_ptr<Equipment> item)
{
	...
	if (index < m_EquipmentSelector.GetData().size())
	{
		...
		m_Context.console.Write(m_CenterScreenW - (item_desc.size() / 2),
						32,
...
void EquipmentMenuState::RenderEquipSlots(int x, int y, const std::wstring& item)
{
	...
	m_Context.console.Write(x, y, item + L":");
	...
	m_Context.console.Write(x + 25, y, equippedItem);
}
...
EquipmentMenuState::EquipmentMenuState(Player& player, StateContext& context)
	: m_Context{ context }
	, m_Player(player)
	, m_MenuSelector{ 
		context.console, 
...
EquipmentMenuState::~EquipmentMenuState() = default;
