...
std::shared_ptr<Item> ItemLoader::CreateObjectFromFile(const std::string& objName)
{
	...
	while (pItem)
	{
		...
		Item::ItemType type = Utilities::ItemTypeFromString(pType->GetText());
		...
		newItem = ItemCreator::CreateItem(type, Utilities::CharToWide(name.c_str()), desc, value, buy_price);
		return newItem;
	}
...
void ItemLoader::SaveObjectToFile(Item* obj)
{
