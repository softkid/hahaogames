...
ShopState::ShopState(StateContext& context, const std::string& shopFilepath)
	: m_Context{ context }
	...
	else if (m_bInItemSell && !m_bBuySellItem)
	{
		...
		if (!m_bSetFuncs)
		{
			if (m_bIsEquipmentShop)
			{
				...
				m_EquipmentSelector.SetData(m_Context.party.GetInventory().GetEquipment());
			}
			else
			{
				...
				m_ItemSelector.SetData(m_Context.party.GetInventory().GetItems());
			}
...
void ShopState::DrawShop()
{
	...
	if (m_pShopParameters->shopType == ShopParameters::ShopType::ITEM)
	{
		...
		m_Context.console.Write(pos_x, 3, L" ___ _                   ____  _                 ", GREEN);
		m_Context.console.Write(pos_x, 4, L"|_ _| |_ ___ _ __ ___   / ___|| |__   ___  _ __  ", GREEN);
		m_Context.console.Write(pos_x, 5, L" | || __/ _ \\ '_ ` _ \\  \\___ \\| '_ \\ / _ \\| '_ \\ ", GREEN);
		m_Context.console.Write(pos_x, 6, L" | || ||  __/ | | | | |  ___) | | | | (_) | |_) |", GREEN);
		m_Context.console.Write(pos_x, 7, L"|___|\\__\\___|_| |_| |_| |____/|_| |_|\\___/| .__/ ", GREEN);
		m_Context.console.Write(pos_x, 8, L"                                          |_|   ", GREEN);
	}
	else if (m_pShopParameters->shopType == ShopParameters::ShopType::WEAPON)
	{
		const int pos_x = m_CenterScreenW - 33;
		m_Context.console.Write(pos_x, 3, L"__        __                             ____  _                 ", GREEN);
		m_Context.console.Write(pos_x, 4, L"\\ \\      / /__  __ _ _ __   ___  _ __   / ___|| |__   ___  _ __  ", GREEN);
		m_Context.console.Write(
			pos_x, 5, L" \\ \\ /\\ / / _ \\/ _` | '_ \\ / _ \\| '_ \\  \\___ \\| '_ \\ / _ \\| '_ \\ ", GREEN);
		m_Context.console.Write(pos_x, 6, L"  \\ V  V /  __/ (_| | |_) | (_) | | | |  ___) | | | | (_) | |_) |", GREEN);
		m_Context.console.Write(pos_x, 7, L"   \\_/\\_/ \\___|\\__,_| .__/ \\___/|_| |_| |____/|_| |_|\\___/| .__/ ", GREEN);
		m_Context.console.Write(pos_x, 8, L"                    |_|                                   |_|   ", GREEN);
	}
	else if (m_pShopParameters->shopType == ShopParameters::ShopType::ARMOR)
	{
		const int pos_x = m_CenterScreenW - 31;
		m_Context.console.Write(pos_x, 3, L"    _                                ____  _                 ", GREEN);
		m_Context.console.Write(pos_x, 4, L"   / \\   _ __ _ __ ___   ___  _ __  / ___|| |__   ___  _ __  ", GREEN);
		m_Context.console.Write(pos_x, 5, L"  / _ \\ | '__| '_ ` _ \\ / _ \\| '__| \\___ \\| '_ \\ / _ \\| '_ \\ ", GREEN);
		m_Context.console.Write(pos_x, 6, L" / ___ \\| |  | | | | | | (_) | |     ___) | | | | (_) | |_) |", GREEN);
		m_Context.console.Write(pos_x, 7, L"/_/   \\_\\_|  |_| |_| |_|\\___/|_|    |____/|_| |_|\\___/| .__/ ", GREEN);
		m_Context.console.Write(pos_x, 8, L"                                                      |_|   ", GREEN);
	}
...
{
	m_Context.console.Write(80, 25, L"QUANTITY:");
	m_Context.console.Write(90, 25, std::to_wstring(m_Quantity));
	m_Context.console.Write(80, 26, L"TOTAL:");
	m_Context.console.Write(90, 26, std::to_wstring(m_Price * m_Quantity));

	// Draw the box
	m_Context.console.DrawPanelHorz(75, 23, 25, BLUE);
	m_Context.console.DrawPanelHorz(75, 32, 25, BLUE);
	m_Context.console.DrawPanelVert(74, 24, 8, BLUE);
	m_Context.console.DrawPanelVert(100, 24, 8, BLUE);
}

void ShopState::DrawItemsBox()
{
	m_Context.console.DrawPanelHorz(25, 14, 41, BLUE);
	m_Context.console.Write(30, 15, L"ITEM:");
	m_Context.console.Write(50, 15, L"PRICE:");

	// Draw the box
	m_Context.console.DrawPanelHorz(26, 16, 40, BLUE);
	m_Context.console.DrawPanelHorz(26, 40, 40, BLUE);
	m_Context.console.DrawPanelVert(65, 15, 27, BLUE);
	m_Context.console.DrawPanelVert(25, 15, 27, BLUE);
	m_Context.console.DrawPanelHorz(25, 42, 41, BLUE);

	const auto& goldStr = std::to_wstring(m_Context.party.GetGold());
	m_Context.console.Write(80, 15, L"GOLD: " + goldStr);
}
...
void ShopState::BuyEquipment()
{
	...
	auto& inventory = m_Context.party.GetInventory();
	...
	// Buy the new item
	if (!m_Context.party.BuyEquipment(m_Quantity * m_Price, std::move(newItem)))
	{
		m_Context.console.Write(80, 34, L"Failed to buy the equipment!", RED);
		return;
	}
}
...
void ShopState::SellEquipment()
{
	...
	m_Context.party.AddGold(totalSellPrice);
	...
	{
		Utilities::remove(m_EquipmentSelector.GetData(), itemIndex);
		Utilities::remove(m_Context.party.GetInventory().GetEquipment(), itemIndex);
	}
}
...
void ShopState::BuyItems()
{
	...
	// Buy the new item
	if (!m_Context.party.BuyItem(m_Quantity * m_Price, std::move(newItem)))
	{
		m_Context.console.Write(80, 34, L"Failed to buy the item!", RED);
		return;
	}
}
...
void ShopState::SellItems()
{
	...
	{
		Utilities::remove(m_ItemSelector.GetData(), itemIndex);
		Utilities::remove(m_Context.party.GetInventory().GetItems(), itemIndex);
	}
}
...
void ShopState::RenderBuyItems(int x, int y, std::shared_ptr<Item> item)
{
	...
	m_Context.console.Write(x, y, name);
	m_Context.console.Write(x + 25, y, price);
}
...
{
	const auto& gold = m_Context.party.GetGold();

	if (m_Context.keyboard.IsKeyJustPressed(KEY_W) && gold >= (m_Quantity + 1) * price)
	{
		m_Quantity++;
	}
	else if (m_Context.keyboard.IsKeyJustPressed(KEY_S) && m_Quantity > 0)
	{
		m_Quantity--;
...
{
	if (m_Context.keyboard.IsKeyJustPressed(KEY_W) && m_Quantity < totalAvailable)
	{
		m_Quantity++;
	}
	else if (m_Context.keyboard.IsKeyJustPressed(KEY_S) && m_Quantity > 0)
