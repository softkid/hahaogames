...
WeaponProperties EquipmentLoader::CreateWeaponProperties(tinyxml2::XMLElement* xmlElement)
{
	...
	WeaponProperties::WeaponType type = Utilities::WeaponTypeFromString(weapon_type_str);
...
ArmorProperties EquipmentLoader::CreateArmorProperties(tinyxml2::XMLElement* xmlElement)
{
	...
	ArmorProperties::ArmorType type = Utilities::ArmorTypeFromString(armor_type_str);
...
StatModifier EquipmentLoader::CreateStatModifier(tinyxml2::XMLElement* xmlElement)
{
	...
	StatModifier::ModifierType mod_type = Utilities::ModifierTypeFromString(mod_type_str);
	...
	StatModifier::ElementalType elemental_type = Utilities::ElementalTypeFromString(elemental_type_str);
...
std::shared_ptr<Equipment> EquipmentLoader::CreateObjectFromFile(const std::string& objName)
{
	...
	while (pItem)
	{
		...
		std::wstring equipName = Utilities::CharToWide(name.c_str());
		...
		Equipment::EquipType equipType{ Utilities::EquipTypeFromString(pType->GetText()) };
		...
		std::wstring desc = Utilities::CharToWide(pDesc->GetText());
