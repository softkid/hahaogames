...
constexpr int STATUS_SIZE = 32;
constexpr int STAT_VAL_X_POS = 70;
constexpr int STAT_LABEL_X_POS = 50;
constexpr int STAT_LABEL_START_Y_POS = 30;
...
void StatusMenuState::DrawStatusPanel()
{
	...
	m_Context.console.Write(menu_x_pos, 2, L"  ____  _        _             ", GREEN);
	m_Context.console.Write(menu_x_pos, 3, L" / ___|| |_ __ _| |_ _   _ ___ ", GREEN);
	m_Context.console.Write(menu_x_pos, 4, L" \\___ \\| __/ _` | __| | | / __|", GREEN);
	m_Context.console.Write(menu_x_pos, 5, L"  ___) | || (_| | |_| |_| \\__ \\", GREEN);
	m_Context.console.Write(menu_x_pos, 6, L" |____/ \\__\\__,_|\\__|\\__,_|___/", GREEN);
...
void StatusMenuState::DrawPlayerInfo()
{
	...
	m_Context.console.Write(m_CenterScreenW - static_cast<int>(player_name.size() / 2), 10, player_name);
	...
	m_Context.console.Write(STAT_LABEL_X_POS, 14, level_str);
	m_Context.console.Write(STAT_LABEL_X_POS, 15, hp_str);
	m_Context.console.Write(STAT_LABEL_X_POS, 16, mp_str);
	m_Context.console.Write(STAT_LABEL_X_POS, 17, xp_str);

	// Draw the equipment and slots
	m_Context.console.Write(STAT_LABEL_X_POS, 19, L"EQUIPMENT", YELLOW);
	m_Context.console.Write(STAT_LABEL_X_POS, 20, L"=========", YELLOW);
	...
	{
		m_Context.console.Write(STAT_LABEL_X_POS, 21 + i, slot2str(slot));
		m_Context.console.Write(STAT_VAL_X_POS, 21 + i, equip ? equip->GetName() : L"EMPTY");
		i++;
	}
	i = 1;

	// Draw the equipment and slots
	m_Context.console.Write(STAT_LABEL_X_POS, STAT_LABEL_START_Y_POS - 2, L"ATTRIBUTES", YELLOW);
	m_Context.console.Write(STAT_LABEL_X_POS, STAT_LABEL_START_Y_POS - 1, L"==========", YELLOW);
...
StatusMenuState::StatusMenuState(Player& player, StateContext& context)
	: m_Context{ context }
	, m_Player{ player }
